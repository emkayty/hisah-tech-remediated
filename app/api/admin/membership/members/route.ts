...
export async function PUT(request: NextRequest) {
  try {
    ...
    const days = payload.durationDays || 30;
    ...
    await database`INSERT INTO membership_activity (actor_id, target_user_id, plan_id, activity_type, details) VALUES (${admin.id}, ${payload.userId}, ${payload.planId}, ${payload.action === 'cancel' ? 'subscription_cancelled' : payload.action === 'grant' ? 'subscription_granted' : 'subscription_extended'}, ${JSON.stringify({ durationDays: days })}::jsonb)`;
